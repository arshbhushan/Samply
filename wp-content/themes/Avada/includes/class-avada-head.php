...
class Avada_Head {
	...
	public function get_og_image() {
		...
		if ( isset( $post->ID ) ) {
			...
			if ( is_array( $post_image ) && isset( $post_image['id'] ) ) {
		...
		if ( $image_id ) {
			...
			if ( is_array( $image ) ) {
				$image['url']    = isset( $image[0] ) ? $image[0] : '';
				$image['width']  = isset( $image[1] ) ? $image[1] : '';
				$image['height'] = isset( $image[2] ) ? $image[2] : '';
				$image['type']   = get_post_mime_type( $image_id );   
			}
		} elseif ( Avada()->settings->get( 'logo' ) ) {
			$image = Avada()->settings->get( 'logo' );
	
			// Handling of logo that haven't been saved.
